...
function areaOfRing(outer, inner) {
    outerArea = Math.PI * outer * outer;
    innerArea = Math.PI * inner * inner;
...
function longestOfThree(value1, value2, value3) {
    ...
    var num = [value1, value2, value3];
    for (var i = 0; i < num.length; i++) {
        if (num[i].length > longString.length) {
            longString = num[i];
        }
    }
    return longString;
...
function stopAt(source, stop) {
    var num = source.indexOf(stop);
    var str = source.substring(0, num);
    return str.trim();
}
...
function capitalize(message) {
    var subStr = message.substring(0, 1).toUpperCase();
    var second = message.substring(1, message.length);
...
/* function calculateTaxRate(salary, status) {
    if ((status === 'single' || status === 'joint') && salary < 74900) {
        if ((status === 'single' && (salary > 1 && salary < 9275)) || (status === 'joint' && (salary > 1 && salary < 18550))) {
            return "10%";
        } else if ((status === 'single' && (salary > 9275 && salary < 37650)) || (status === 'joint' && (salary > 18550 && salary < 75300)) {
            return "15%";
        } else if (status === 'single' && (salary > 37650 && salary < 74900)) {
            return "25%";
        }
    } else {
